...
test_that("T1) addresses of objects referenced through an environment (e.g. env1$x) are correctly returned", {
  ...
  expect_equal(as.vector(addresses), c(address(globalenv()$env1$x), address(globalenv()$env1$y)))
...
test_that("T3) the address of an object passed as a string is correctly returned (in different environments)", {
  # skip ("not now")
  # browser()
  expect_equal(get_obj_address("env1"), address(globalenv()$env1))
  expect_equal(get_obj_address("x", envir=globalenv()$env1), address(globalenv()$env1$x))
})
...
test_that("T4) the address of an object passed as an object is correctly returned (in different environments)", {
  ...
  # browser()
  expect_equal(get_obj_address(env1), address(globalenv()$env1))
  expect_equal(get_obj_address(env11, envir=globalenv()$env_of_envs), address(globalenv()$env_of_envs$env11))
  expect_equal(get_obj_address(x, envir=globalenv()$env1), address(globalenv()$env1$x))
})
...
test_that("T5) the address of objects passed as expressions is correctly returned", {
  # skip ("not now")
  # browser()
  expect_equal(get_obj_address(globalenv()$objects[1], envir=globalenv()$env1), address(globalenv()$env1$x))
})
...
test_that("T9) the address returned for an object stored in a deeply nested environment is correct", {
  # skip ("not now")
  # browser()
  expect_equal(get_obj_address("z", envir=globalenv()$env_of_envs$env11), address(globalenv()$env_of_envs$env11$z))
})
