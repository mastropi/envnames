...
Clearly the last one is the result we most likely want, and the envnames makes this possible by creating a lookup table that maps environment names to their memory addresses. Using this lookup table, it is possible to retrieve the name of any environment where an object resides, _even_ the name of function execution environments (in which case, the name is given by the function itself).
...
For more information and examples, see function `get_obj_name()`, `get_obj_value()`, which can be used to retrieve the name and value of the variable leading to a particular variable in derived or nested environments, and `environment_name()`, which can be used to retrieve the name of an execution environment.  
...
This section describes the functionality of 8 selected functions out of the 11 available functions (sorted by relevance):  
...
2) `environment_name()` / `get_env_name()` (its alias), used to get the name of user-defined and execution environments.  

3) `obj_find()`, used to find an object in the workspace and recursively within environments.  

4) `get_fun_calling_chain()`, used to get the function calling stack displayed in a format that is easier than the one provided by `sys.calls()`.  

5) `get_obj_name()`, used to retrieve the _name_ of the object leading to a given function's parameter.  

6) `get_obj_value()`, used to retrieve the _value_ of the object leading to a given function's parameter.  
...
The following three functions are not yet covered by the vignette, but they can be consulted via their manual pages:  
...
The title of each sub-section is a sentence stating the main purpose of the presented function.

## 1) `get_env_names()`: retrieve the address-name lookup table of defined environments
The `get_env_names()` function is used to retrieve the address-name lookup table that is used by the other functions in the package to retrieve environment names from their memory addresses.
...
The main columns defining the lookup are `address` (containing the memory address of the environment) and `name` (containing the name of the environment). The other columns are used to give context to the environments, such as their `location`, which for instance could indicate the package where an environment is defined, or the environment where a function is defined (if the concerned environment is a function's execution environment).

We can also restrict the lookup table to the environments (including function execution environments) defined within another environment: 
...
If called from within a function with no arguments, `environment_name()` returns the execution environment of the function. Th execution environment name is the name of the function given with its *full path*, as in e.g. `env1$f` (when `environment_name()` is called from function `f()` defined in environment `env1`).
...
env_of_envs$env21$f()
cat("\n...and when we call f() from function g(),
    we get the output that follows:\n")
env_of_envs$env21$g()
```
Note that, in the second case, the enviroment showing as path to `f()` is *not* `env_of_envs$env21` (as we would have expected) but `e_proxy`. The reason is that environment `e_proxy` (in the global environment) points to environment `env_of_envs$env21`. And since environment names are retrieved by their memory address  (which in this case is the memory address of `f`'s execution environment), there may be more than one environment matching the same memory address. In such cases, the rule is to retrieve the matching environment whose name comes first in alphabetical order (which in this case is `e_proxy` --which comes before `env_of_envs$e_proxy` and before `env_of_envs$env21`, which all match the memory address of the environment where `f()` is defined).

But if we call `env_of_envs$env21$f()` from a function `h()` defined in the `env_of_envs$env21` environment, we get:
...
## 5) `get_obj_name()`: retrieve the name of an object at a specified parent generation
The `get_obj_name()` function is intended to be called from within a function. It can be used to know the name of the object that *leads* to a particular parameter a few generations back following the function calling chain. In other words, sometimes it is useful to know what object in a given parent generation environment is responsible for a function's parameter value.

At first, one may have the impression that the result of this function is the same as the one provided by `deparse(substitute())`. However, this is not the case and the examples following show the differences and similarities between the two.
...
getObjNameAndCompareWithSubstitute <- function(y, eval=FALSE) {
  ...
  eval_result = eval(quote(y), envir=parent.frame(n=parent_generation))
  if (!eval) {
    ...
    cat("Result of deparse(substitute(y, parent.frame(n=", parent_generation, "))): ", deparse_substitute_result,
        "\n\tConceptually this is the substitution of *variable* 'y' at parent generation ", parent_generation,
        "\n\tconverted to a string.\n", sep="")
  } else {
    cat("Result of get_obj_name(y, n=", parent_generation, ", eval=", eval, "): ", get_obj_name_result,
        "\n\tConceptually this is the object LEADING to *parameter* 'y' evaluated at parent generation ",
        parent_generation, ".\n", sep="")
    cat("Result of deparse(y): ", deparse_result,
        "\n\tConceptually this is the value of *parameter* 'y' converted to a character string.",
        "\n", sep="")
    cat("Result of substitute(y, parent.frame(n=", parent_generation, ")): ", substitute_result,
        "\n\tConceptually this is the substitution of *variable* 'y' at parent generation ", parent_generation,
        ".\n", sep="")
    cat("Result of eval(quote(y), envir=parent.frame(n=", parent_generation, ")): ", eval_result,
        "\n\tConceptually this is the evaluation of *variable* 'y' at parent generation ", parent_generation,
        ".\n", sep="")
  }
}

callGetObjNameAndCompareWithSubstitute <- function(x, eval=FALSE) { getObjNameAndCompareWithSubstitute(x, eval=eval) }
```
...
y <- -9   # Global variable with the same name as the parameter of function getObjNameAndCompareWithSubstitute()
z <- 3
...
When `eval=TRUE`, `get_obj_name()` behaves the same way as `deparse()`, because the values of the objects leading to parameter `y` in parent generations is always the same and equal to the parameter's value. The following example illustrates:
```{r GetObjNameExampleCall2}
y <- -9
z <- 3;
callGetObjNameAndCompareWithSubstitute(z, eval=TRUE)
```
That is, calling `get_obj_name(y, n=n, eval=TRUE)` retrieves the value of parameter `y` in parent generation `n`, which is the same in all parent generations and equal to the value of parameter 'y' inside the calling function. Therefore, this is the same as the result of `deparse(y)`.
...
## 6) `get_obj_value()`: retrieve the value of an object at a specified parent generation 
The result of calling this function is the same as calling `get_obj_name` with `eval=TRUE`.

The following example illustrates this, showing its difference with the result of using `eval()`.
...
y <- -9   # Global variable with the same name as the parameter of function getObjValueAndCompareWithEval()
z <- 3
...
# Summing up
We have described the following 8 functions of the `envnames` package and shown examples of using them:  
...
5) `get_obj_name()` to retrieve the name of the object leading to a given function's parameter.

6) `get_obj_value()` to retrieve the value of the object leading to a given function's parameter.
