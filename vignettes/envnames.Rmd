...
This section describes the functionality of the 11 available functions (sorted by relevance from a mix of historical and practical use view):  
...
In the following sub-sections, the title is a sentence stating the main purpose of the presented function.

## 1) `get_env_names()`: retrieve the address-name lookup table of defined environments
The `get_env_names()` function returns the address-name lookup table that is the basis for most of the other functions in the package that rely on it to retrieve the names of environments based on their memory addresses.
...
The two main columns defining the lookup table are `address` (containing the memory address of the environment) and `name` (containing the name of the environment). The other columns are used to give context to the environments, such as their `location`, which for instance could indicate the package where an environment is defined, or the environment where a function is defined (if the concerned environment is a function's execution environment).
...
If called from within a function with no arguments, `environment_name()` returns the execution environment of the function. The execution environment name is the name of the function given with its *full path*, as in e.g. `env1$f`, when `environment_name()` is called from function `f()` defined in environment `env1`.
...
But if we call `env_of_envs$env21$f()` from a function `h()` defined in the `env_of_envs$env21` environment (as shown next), we get:
...
### Basic operation
Let's start defining a couple of functions that form a function calling chain. The called function `h()` retrieves the memory address of its callee.
...
The `get_obj_name()` and `get_obj_value()` functions are intended to help track objects and their values as they are passed through different environments. The most useful of the two is `get_obj_name()`, because the *values* of linked objects are all the same as they traverse the different environments, making `get_obj_value()` be almost the same as calling `deparse()` at any environment (except for some special cases described in the documentation).
...
After learning a little more about `get_obj_name()`, one may have the impression that it gives the same result as the one provided by `deparse(substitute())`. However, this is not the case as is shown in the examples of the next section.
