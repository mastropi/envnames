...
#### Basic operation
The following call returns a data frame containing the address-name lookup table, where the two main columns are `address` (containing the memory address of the environment) and `name` (containing the name of the environment). The other columns are used to give context to the environments, such as `path` which tells us how to get to nested user-defined environments (see the case for environment `env21` nested within `env_of_envs`), and `location` which indicates, for instance, the package where an environment is defined or the environment where a function is defined (if the concerned environment is a function's execution environment).
...
- the output from the first call includes *all* the environmnts where the given environment is found.

- the output from the second call contains the *path* to the environment being searched for stating that `env21` is found inside environment `env_of_envs`.
...
Note that no path information is attached to the returned names because only one environment is found inside the specified environments.
...
i.e., when making explicit the location of function `f()`, such location is shown as part of the name of the execution environment (as opposed to seeing location `e_proxy` as above).

## 3) `obj_find()`: find the environment where (visible) objects exist
With the `obj_find()` function we can check if an object exists in an environment of the workspace and retrieve all the environments where it has been found. In the case of packages, only *exported* objects can be found.

All environments --including system environments, packages, user-defined environments, and optionally function execution environments-- are crawled and searched for the object. This includes any environments that are defined *within* other environments (*nested*), and it represents an enhancement to the built-in `exists()` function, which does *not* search for an object inside nested environments.
...
print(environments_where_objs_are_found)
```
NOTE: Even if we run `sapply()` inside environment `env1`, it's important to add parameter `envir=env1` to the call to `obj_find()`; if we don't add it, *no object is found* because the parent environment of the call to `obj_find()` is *not* `env1` but the `sapply()` execution environment, where the objects do not exist.
...
Following is an example that shows the difference between using the `get_fun_calling_chain()` function vs. the built-in `sys.call()` function to retrieve the calling stack. 
...
h <- function(x) {
  ...
  cat("Using get_fun_calling() and environment_name() functions:
      The parent frame of function", get_fun_name(), "is", get_fun_calling(n=1), "\n")
  xval_h = x
...
## 7) `get_obj_address()` and `address()`: retrieve the memory address of an object
...
4) We have used `get_fun_name()`, `get_fun_calling()`, `get_fun_calling_chain()` to get the stack of calling functions. These functions return the stack information in a manner that is much simpler than the built-in `sys.calls()` function, making it easier to check the_*names* of the calling functions.  

5) We have used `get_fun_env()` to get the execution environment of a given function in the calling stack, so that we can query objects that live within.  
